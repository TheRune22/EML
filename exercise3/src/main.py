...
model = nn.Sequential(
    ...
    block(activation, in_channels=1, out_channels=32,
          kernel_size=3, padding='same'),
    block(activation, in_channels=32, out_channels=32,
          kernel_size=3, padding='same'),
    SkipConnection(
        nn.MaxPool2d(2),
        block(activation, in_channels=32, out_channels=64,
              kernel_size=3, padding='same'),
        block(activation, in_channels=64, out_channels=64,
              kernel_size=3, padding='same'),
        SkipConnection(
            nn.MaxPool2d(2),
            block(activation, in_channels=64, out_channels=128,
                  kernel_size=3, padding='same'),
            block(activation, in_channels=128, out_channels=128,
                  kernel_size=3, padding='same'),
            SkipConnection(
                nn.MaxPool2d(2),
                block(activation, in_channels=128, out_channels=256,
                      kernel_size=3, padding='same'),
                block(activation, in_channels=256, out_channels=256,
                      kernel_size=3, padding='same'),
                SkipConnection(
                    nn.MaxPool2d(2),
                    block(activation, in_channels=256, out_channels=512,
                          kernel_size=3, padding='same'),
                    block(activation, in_channels=512, out_channels=512,
                          kernel_size=3, padding='same'),
                    SkipConnection(
                        nn.MaxPool2d(2),
                        block(activation, in_channels=512, out_channels=1024,
                              kernel_size=3, padding='same'),
                        block(activation, in_channels=1024, out_channels=1024,
                              kernel_size=3, padding='same'),
                        nn.ConvTranspose2d(in_channels=1024, out_channels=1024,
                                           kernel_size=2, stride=2,
                                           output_padding=(int(input_height / 16) % 2,
                                                           int(input_width / 16) % 2)),
                    ),
                    block(activation, in_channels=512+1024, out_channels=512,
                          kernel_size=3, padding='same'),
                    block(activation, in_channels=512, out_channels=512,
                          kernel_size=3, padding='same'),
                    nn.ConvTranspose2d(in_channels=512, out_channels=512,
                                       kernel_size=2, stride=2,
                                       output_padding=(int(input_height / 8) % 2,
                                                       int(input_width / 8) % 2)),
                ),
                block(activation, in_channels=256+512, out_channels=256,
                      kernel_size=3, padding='same'),
                block(activation, in_channels=256, out_channels=256,
                      kernel_size=3, padding='same'),
                nn.ConvTranspose2d(in_channels=256, out_channels=256,
                                   kernel_size=2, stride=2,
                                   output_padding=(int(input_height / 4) % 2,
                                                   int(input_width / 4) % 2)),
            ),
            block(activation, in_channels=128+256, out_channels=128,
                  kernel_size=3, padding='same'),
            block(activation, in_channels=128, out_channels=128,
                  kernel_size=3, padding='same'),
            nn.ConvTranspose2d(in_channels=128, out_channels=128,
                               kernel_size=2, stride=2,
                               output_padding=(int(input_height / 2) % 2,
                                               int(input_width / 2) % 2)),
        ),
        block(activation, in_channels=64+128, out_channels=64,
              kernel_size=3, padding='same'),
        block(activation, in_channels=64, out_channels=64,
              kernel_size=3, padding='same'),
        nn.ConvTranspose2d(in_channels=64, out_channels=64,
                           kernel_size=2, stride=2,
                           output_padding=(input_height % 2,
                                           input_width % 2)),
    ),
    block(activation, in_channels=32+64, out_channels=32,
          kernel_size=3, padding='same'),
    block(activation, in_channels=32, out_channels=32,
          kernel_size=3, padding='same'),
